...
Calculamos las medias muestrales ($\bar{x}$, $\bar{y}$), las matrices de covariancia muestrales ($S_1$, $S_2$) y la matriz $\hat{S}$.
...
Evaluamos si el vector de medias de las variables cuantitativas `gasto_persona` y `tiempo_transporte` es el mismo para dos niveles distintos de la variable no cuantitativa `destino` que hemos modelado como multinomial: viaje solo por España (`solo ESP`) y viaje solo por el estrangero (`solo extranjero`).
...
Del Análisis Descriptivo, destacamos:
  ...
  - A mayor duración del viaje mayor tiempo de transporte, lo cual es razonable.
